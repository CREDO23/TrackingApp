...
module.exports = (app) => {
    app.get('/api/notification/planeDocking/:user', body_parser.json() , async (req , res) => {
     ...
     await   shipping.find({mobile:mobile , state : "En cours"}).then( async (element) =>{ 
       ...
       element.filter(element => element.destinationCity == ville  ).forEach( async (data) => {
        await shipping.updateOne(data , {state : "Terminée"} , {new:true}).then(data => console.log(data))
  })
         }
          ...
          arraySender.forEach( async data => {
            await  customer.findOne({userName : data}).then(  (user) => {
             i++ 
                ...
                sendEmail(arraySender[i] , arrayRecipient[i] , destinationCity[i] , ville , false , user.eMail ) 
          ...
          arrayRecipient.forEach( async data => {
            await  customer.findOne({userName : data}).then((user) => {
                j++
                    ...
                    sendEmail(arraySender[j] , arrayRecipient[j] , destinationCity[j] , ville , false , user.eMail )   
