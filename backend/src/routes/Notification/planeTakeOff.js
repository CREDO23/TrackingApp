...
module.exports = (app) => {
    app.get('/api/notification/planeTakeOff/:user', body_parser.json() , async (req , res) => {
        ...
        if(arraySender[0]) {
          arraySender.forEach( async data => {
            await  customer.findOne({userName : data}).then( (user) => {
             i++ 
             sendEmail(arraySender[i] , shippingCity[i] , ville , true , user.eMail) 
            })
         ...
         if(arrayRecipient[0]) {
          arrayRecipient.forEach( async data => {
            await  customer.findOne({userName : data}).then((user) => {
                j++  
                sendEmail(arrayRecipient[j] , shippingCity[j] , ville , false , user.eMail)  
          }) 
       })
         }
