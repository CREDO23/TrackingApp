...
const trackPackage = (app) => {
	app.get(
		'/api/customer/track/:key',
		body_parser.json(),
		async (req, res) => {
			if (!ID.isValid(req.params.key))
				return res
					.status(400)
					.json('La clé doit avoir 24 caractères');
	...
	app.get(
		...
		async (req, res) => {
			...
			if (history[0]) {
				...
				res
					.status(400)
					.json("Vous n'avez encore effectuez aucune transaction !");
			}
		},
	);
};
