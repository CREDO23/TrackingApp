	...
	app.post(
		...
		async (req, res) => {
			if (req.body.password) {
				...
				res
					.status(400)
					.json({ password: 'Renseigner votre mot de pass' });
			}
		},
	);
};
...
const updateCustomer = (app) => {
	app.put(
		'/api/updateCustomer/:userName',
		body_parser.json(),
		async (req, res) => {
			const user = req.params.userName;
			await customer
				.findOneAndUpdate({ userName: user }, req.body, { new: true })
				.then((data) => {
