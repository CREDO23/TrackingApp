...
const createShipping = (app) => {
	app.post(
		...
		async (req, res) => {
			...
			await customer
				.findOne({ userName: req.body.sender })
				.then((data) => {
			...
			if (a && b && a != b) {
				await shipping
					...
					.then((shipping) => {
						...
						customer
							.findOne({ userName: shipping.sender })
							.then((user) =>
								send(user.eMail, shipping, shipping._id),
							);
						customer
							.findOne({ userName: shipping.recipient })
							.then((user) =>
								send(user.eMail, shipping, shipping._id),
							);
				...
				if (!a)
					res
						.status(404)
						.json(
							"L'éxpediteur n'est pas trouvé dans la base des données",
						);
				if (!b)
					res
						.status(404)
						.json(
							"Le déstinataire n'est pas trouvé dans la base des données",
						);
				if (a == b)
					res
						.status(400)
						.json(
							"Le destinataire doit etre different de l'éxpediteur",
						);
			}
		},
	);
